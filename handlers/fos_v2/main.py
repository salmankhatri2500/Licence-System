"""
main.py  –  FOS v2
Faiz Online Service Bot

Group architecture:
  group=0 → ConversationHandlers (highest priority)
  group=1 → message_router (only fires outside conversations)

This prevents "Apna panel" / "Button use karein"
...
try:
    ...
    print("ERROR: GOOGLE_CREDS_JSON invalid JSON!")
    exit(1)
...
from handlers.admin import (add_agent_start, aa_name, aa_phone, aa_tid,
                             aa_rate, aa_sheet, find_agent,
                             admin_bc_start, admin_bc_send)
...
async def post_init(app):
    register_jobs(app)
    logger.info("🚀 FOS v2 Bot started!")


async def debug_cmd(update, ctx):
    if update.effective_user.id != SUPER_ADMIN_ID: return
    from db import all_agents, queue_today_count, get_admin_rate, get_admin_qr, all_operators, operators_enabled
    ...
    ops = all_operators()
    lines.append(f"\n👥 Operators: {len(ops)} ({'ON' if operators_enabled() else 'OFF'})")
    lines.append(f"💰 Admin rate: Rs{get_admin_rate()}/app")
    lines.append(f"🖼 QR: {'✅' if get_admin_qr() else '❌'}")
...
def build_app():
    ...
    app.add_handler(ConversationHandler(
        ...
        allow_reentry=True, name="reg"), group=0)
    ...
    app.add_handler(ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cmd_cancel)],
        allow_reentry=True, name="add_agent"), group=0)
    ...
    app.add_handler(ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cmd_cancel)],
        allow_reentry=True, name="new_app"), group=0)
    ...
    app.add_handler(ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cmd_cancel)],
        allow_reentry=True, name="pay_admin"), group=0)
    ...
    app.add_handler(ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cmd_cancel)],
        allow_reentry=True, name="bc"), group=0)
    ...
    app.add_handler(ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cmd_cancel)],
        allow_reentry=True, name="abc"), group=0)
    ...
    app.add_handler(ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cmd_cancel)],
        allow_reentry=True, name="ur"), group=0)
    ...
    # ══════════════════════════════════════════════════════════
    # GROUP 1: General router (lower priority)
    # ══════════════════════════════════════════════════════════
    app.add_handler(
...
def main():
    ...
    logger.info("🤖 Starting FOS v2 Bot…")
    app = build_app()
    logger.info("✅ FOS v2 is running!")
