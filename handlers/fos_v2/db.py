...
Sheets in FOS_Master:
  agents | logs | queue | admin_settings | operators
Agent sheet tabs:
...
from config import GOOGLE_CREDS, SCOPES, MASTER_SHEET
...
HDR = {
    ...
    "queue":  ["queue_id","app_no","dob","password","client_code",
               ...
               "agent_tid","submitted_at","status","done_at","priority","done_by"],
    "admin_settings": ["key","value"],
    "operators": ["op_id","op_name","phone","telegram_id","added_at","status"],
...
COL_W = {
    ...
    "queue":        [130,110,90,130,120,150,120,110,150,120,150,80,150,70,120],
    "operators":    [100,160,120,140,150,80],
...
class DB:
    ...
    def connect(self) -> bool:
        try:
            ...
            self._msh = None
            logger.info("DB: connected to Google Sheets")
            return True
    ...
    def ensure(self, sh, tab: str):
        ...
        try:
            ...
            pass
        try:
            w = sh.add_worksheet(title=tab, rows=5000, cols=25)
            if tab in HDR:
                w.append_row(HDR[tab])
                self._fmt(w, tab)
            return w
        except Exception as e:
            logger.error(f"DB.ensure({tab}): {e}")
            return None
    ...
    def _fmt(self, ws, tab: str):
        try:
            ...
            r.append({"updateDimensionProperties":{
                "range":{"sheetId":wid,"dimension":"ROWS","startIndex":0,"endIndex":5000},
    ...
    def update(self, ws, col: int, key: str, field: str, val) -> bool:
        try:
            ...
            logger.error(f"DB.update({field}={val}): {e}")
        return False
...
def _opw():
    sh = db.master()
    return db.ensure(sh, "operators") if sh else None
...
def get_admin_rate()   -> float: return safe_float(get_admin_setting("rate_per_app"))
def get_admin_qr()     -> str:   return get_admin_setting("qr_file_id")
def operators_enabled()-> bool:  return get_admin_setting("operators_enabled") == "1"
def reminders_enabled()-> bool:  return get_admin_setting("reminders_enabled") != "0"


# ════════════════════════════════════════════════════════════════
# OPERATORS
# ════════════════════════════════════════════════════════════════
def all_operators() -> list:
    w = _opw()
    return db.rows(w) if w else []

def operator_by_tid(tid: int):
    s = str(tid).strip()
    for op in all_operators():
        if str(op.get("telegram_id","")).strip() == s:
            return op
    return None

def operator_by_phone(phone: str):
    for op in all_operators():
        if op.get("phone","").strip() == phone.strip():
            return op
    return None

def add_operator(data: dict) -> bool:
    try:
        w = _opw()
        if not w: return False
        row = [data["op_id"], data.get("op_name",""), data["phone"],
               str(data.get("telegram_id","")), now_ist(), "active"]
        w.append_row(row)
        n = len(w.get_all_values())
        db.color_row(w, n, "active")
        return True
    except Exception as e:
        logger.error(f"add_operator: {e}")
        return False

def remove_operator(op_id: str) -> bool:
    try:
        w  = _opw()
        if not w: return False
        ri, _ = db.find(w, 0, op_id)
        if ri:
            w.delete_rows(ri)
            return True
    except Exception as e:
        logger.error(f"remove_operator: {e}")
    return False

def set_operator_field(op_id: str, field: str, val) -> bool:
    w = _opw()
    return db.update(w, 0, op_id, field, val) if w else False

def is_operator(tid: int) -> bool:
    if not operators_enabled(): return False
    return operator_by_tid(tid) is not None
...
def agent_by_phone(phone: str):
    for a in all_agents():
        if a.get("phone","").strip() == phone.strip():
            return a
    return None
...
def deduct_agent_balance(agent_id: str, amount: float) -> bool:
    ...
    return set_agent_field(agent_id, "balance", new)
...
def queue_mark_done(queue_id: str, done_by: str = "admin") -> bool:
    w  = _qw()
    if not w: return False
    ok = db.update(w, 0, queue_id, "status",  "DONE")
    db.update(w, 0, queue_id, "done_at", now_ist())
    db.update(w, 0, queue_id, "done_by", done_by)
    if ok:
...
def queue_today_count() -> dict:
    today = __import__("utils").today_ist()
...
def queue_stats_range(days: int = 30) -> dict:
    """Stats for last N days."""
    from datetime import date, timedelta
    cutoff = (date.today() - timedelta(days=days)).strftime("%Y-%m-%d")
    items  = [r for r in queue_all() if r.get("submitted_at","") >= cutoff]
    return {
        "total":   len(items),
        "done":    sum(1 for r in items if r.get("status") == "DONE"),
        "pending": sum(1 for r in items if r.get("status") == "PENDING"),
        "held":    sum(1 for r in items if r.get("status") == "HELD"),
    }
...
def detect_role(tid: int) -> str:
    ...
    c, _ = find_client(tid)
    if c:                               return "client"
    if is_operator(tid):                return "operator"
    return "unknown"
